...
const Sidebar: React.FC<IProps> = props => {
  ...
  const selectTaskList = (index: number) => {
    ...
    if (index >= 0) selectedTaskListChanged(taskLists[index].id, taskLists[index].title);
    else selectedTaskListChanged('all', 'All Tasks');
  };
  ...
  return (
    <div>
      ...
      <NameModal
        ...
        handleClose={() => setEditingTaskList(false)}
        handleNameChanged={insertTaskList}
      />

      <Divider />

      <List component="nav" aria-label="Main Actions">
        <ListItem button selected={selectedIndex === -1} onClick={() => selectTaskList(-1)}>
          <ListItemIcon>
            <ListIcon />
          </ListItemIcon>
      ...
      <List component="nav" aria-label="Task Lists">
        ...
        <ListItem button component="a" href="https://github.com/bajcmartinez/google-tasks-ui" target="_blank">
          <ListItemIcon>
